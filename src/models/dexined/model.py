...
Vendored from https://github.com/xavysp/DexiNed (MIT License).
Original paper: Soria et al., "Dense Extreme Inception Network: Towards a
Robust CNN Model for Edge Detection", WACV 2020.
"""
...
class UpConvBlock(nn.Module):
    ...
    def _make_deconv_layers(self, in_features, up_scale):
        ...
        for i in range(up_scale):
            ...
            out_features = self._compute_out_features(i, up_scale)
    ...
    def _compute_out_features(self, idx, up_scale):
...
class DexiNed(nn.Module):
    ...
    def forward(self, x):
        ...
        block_cat = torch.cat(results, dim=1)   # [B, 6, H, W]
        block_cat = self.block_cat(block_cat)   # [B, 1, H, W]

        results.append(block_cat)
        return results  # list of 7 tensors, last is fused output
